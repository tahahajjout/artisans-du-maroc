...
// ── Cloudinary storage for product images and gallery ──
const storage = new CloudinaryStorage({
  cloudinary,
  params: async (req, file) => ({
    folder: "artisans-du-maroc/products",
    resource_type: file.mimetype.startsWith("video") ? "video" : "image",
    allowed_formats: ["jpg", "jpeg", "png", "webp", "mp4", "mov"],
    transformation: file.mimetype.startsWith("video")
      ? []
      : [{ width: 800, quality: 80, fetch_format: "auto" }],
  }),
});
  ...
  try {
    ...
    if (!title || !price || !category_id || !artisan_id || !image_url) {
      return res.status(400).json({
        message:
          "Tous les champs marqués d'une étoile (*) sont obligatoires, y compris l'image.",
      });
    }
    ...
    // Insert gallery files — Cloudinary returns full URL in .path
    const galleryFiles = req.files?.["gallery"] || [];
    for (const file of galleryFiles) {
      const fileType = file.mimetype.startsWith("video") ? "video" : "image";
      await db.execute(
        "INSERT INTO product_gallery (product_id, file_url, file_type) VALUES (?, ?, ?)",
        [productId, file.path, fileType],
      );
    }

    res
      .status(201)
      .json({ message: "Produit ajouté avec succès !", productId });
  } catch (error) {
    console.error("Erreur SQL détaillée :", error);
    res
      .status(500)
      .json({ message: "Erreur lors de la sauvegarde en base de données." });
  }
};
...
exports.updateProduct = async (req, res) => {
  ...
  try {
    ...
    if (req.files?.["image"]?.[0]) {
      sets.push("image_url = ?");
      params.push(req.files["image"][0].path);
    }
    ...
    // Insert new gallery files — Cloudinary returns full URL in .path
    const galleryFiles = req.files?.["gallery"] || [];
    for (const file of galleryFiles) {
      const fileType = file.mimetype.startsWith("video") ? "video" : "image";
      await db.execute(
        "INSERT INTO product_gallery (product_id, file_url, file_type) VALUES (?, ?, ?)",
        [id, file.path, fileType],
      );
    }
...
exports.rateProduct = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Erreur lors de l'enregistrement de la note." });
  }
};
...
exports.getRecommendations = async (req, res) => {
  ...
  try {
    ...
    const cat2Products =
      catVisits.length > 1
        ? await fetchFromCategory(catVisits[1].category_id)
        : [];
    ...
    let allProducts = [
      ...cat1Products.slice(0, take1),
      ...cat2Products.slice(0, take2),
    ];
    ...
    if (remaining > 0) {
      ...
      const excludedCatIds =
        catVisits.map((c) => c.category_id).join(",") || "0";
...
// ── GET ARTISAN PRODUCT VISITS FOR ADMIN ─────────────────────────────────────
exports.getArtisanProductVisits = async (req, res) => {
  const { artisanId } = req.params;
  try {
    const [rows] = await db.execute(
      `
