...
exports.getTopCities = async (req, res) => {
    try {
        const [rows] = await db.query('SELECT city, COUNT(*) as count FROM artisans GROUP BY city ORDER BY count DESC LIMIT 8');
        res.json(rows);
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
exports.getArtisanById = async (req, res) => {
    const { id } = req.params;
    try {
        const [rows] = await db.execute("SELECT id, full_name, city, email, bio, profile_picture, banner_photo, phone_number FROM artisans WHERE id = ?",[id]);
...
exports.updateArtisan = async (req, res) => {
    try {
        ...
        let sets = ['full_name = ?', 'email = ?', 'city = ?', 'bio = ?', 'phone_number = ?'];
        let params = [full_name, email, city, bio, phone_number];

        if (newProfilePic) { sets.push('profile_picture = ?'); params.push(newProfilePic); }
        if (newBanner)     { sets.push('banner_photo = ?');    params.push(newBanner); }
...
exports.changePassword = async (req, res) => {
    ...
    try {
        const [rows] = await db.execute('SELECT password FROM artisans WHERE id = ?', [id]);
        ...
        await db.execute('UPDATE artisans SET password = ? WHERE id = ?', [new_password, id]);
...
exports.registerArtisan = async (req, res) => {
    try {
        ...
        await db.query(insertQuery, [
            full_name, email, password, bio, city, phone_number, profile_picture, banner_photo
        ]);
        ...
        console.error("Erreur critique lors de l'inscription de l'artisan :", error);
        res.status(500).json({ 
            success: false, 
...
exports.deleteArtisan = async (req, res) => {
    ...
    try {
        ...
        if (rows.length === 0) return res.status(404).json({ success: false, message: "Artisan non trouvé." });
...
exports.search = async (req, res) => {
    const { q } = req.query;
    if (!q || q.trim() === '') return res.status(400).json({ message: 'Terme vide.' });
