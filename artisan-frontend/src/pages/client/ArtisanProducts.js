...
function ArtisanProducts() {
  ...
  useEffect(() => {
    axios
      .get(`${process.env.REACT_APP_API_URL}/api/products/artisan/${id}/public`)
      .then((res) => {
  ...
  const whatsappLink = (phone, productTitle) => {
    ...
    const message = encodeURIComponent(
      `Bonjour, je suis intéressé par le produit : ${productTitle}`,
    );
  ...
  const submitRating = async () => {
    if (stars === 0) {
      setRatingMsg("Veuillez sélectionner une note.");
      return;
    }
    setSubmitting(true);
    try {
    ...
    <div className="ap-page">
      ...
      <div className="ap-section-title">
        <h2>
          MES CRÉATIONS:
          <br />
          <span>{artisanName.toUpperCase()}</span>
        </h2>
      </div>
      ...
      <div className="ap-filter-bar">
        <button
          className={`ap-filter-toggle ${showFilters ? "active" : ""}`}
                ...
                <div className="ap-card-actions">
                  <a
                    ...
                    onClick={() => {
                      ...
                      axios.post(
                        `${process.env.REACT_APP_API_URL}/api/products/${p.id}/visit`,
                        {
                  ...
                  <button
                    className="ap-comments-btn"
                    onClick={(e) => openComments(e, p)}
                  >
                    💬 Avis
                  </button>
                </div>
        ...
        <div className="ap-modal-overlay" onClick={() => setRatingModal(null)}>
          <div className="ap-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="ap-guest-msg">
              ...
              <a href="/login" className="ap-login-link">
                Se connecter
              </a>
            </div>
          </div>
      ...
      {galleryModal && (
        ...
        >
          <div
            className="ap-modal ap-gallery-modal"
          ...
          >
            ...
            <div style={{ marginBottom: "14px" }}>
              ...
              <img
                src={imgSrc(galleryModal.image_url)}
                alt="main"
                className="ap-gallery-main-img"
                onClick={() =>
                  setGalleryLightbox({
                    url: imgSrc(galleryModal.image_url),
                    type: "image",
                  })
                }
                onError={(e) =>
            ...
            <div className="ap-gallery-details">
              <span className="ap-detail-tag">
                🎨 {galleryModal.couleur || "Non spécifié"}
              </span>
              <span className="ap-detail-tag">
                ↕ H: {galleryModal.hauteur || "Non spécifié"}
              </span>
              <span className="ap-detail-tag">
                ↔ L: {galleryModal.largeur || "Non spécifié"}
              </span>
              <span className="ap-detail-tag">
                🧵 {galleryModal.matiere || "Non spécifié"}
              </span>
            </div>
            ...
            {galleryModal.gallery && galleryModal.gallery.length > 0 ? (
              <>
                <p
                  className="ap-gallery-section-label"
                  style={{ marginTop: "14px" }}
                >
                  Galerie ({galleryModal.gallery.length} média
                  {galleryModal.gallery.length > 1 ? "s" : ""})
                </p>
