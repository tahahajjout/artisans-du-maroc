...
const CITIES = [
    ...
    'Kénitra','Tétouan','Safi','El Jadida','Béni Mellal','Nador','Khouribga',
    'Settat','Al Hoceïma','Essaouira','Taza','Guelmim','Laayoune','Dakhla',
    'Ifrane','Chefchaouen','Autre...'
];

function ArtisanRegister() {
    const [formData, setFormData] = useState({
        full_name: '', email: '', password: '', bio: '', city: '', phone_number: ''
    });
    ...
    const handleProfileFile = (e) => {
        ...
        if (f) { setProfilePicture(f); setProfilePreview(URL.createObjectURL(f)); }
    };
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!formData.city) { setError("Veuillez sélectionner votre ville."); return; }
