...
function Home() {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_API_URL}/api/search?q=${encodeURIComponent(searchTerm)}`,
      );
  ...
  const nextSlide = () => {
    if (currentIndex + 4 < artisans.length) setCurrentIndex(currentIndex + 4);
  };
  const prevSlide = () => {
    if (currentIndex - 4 >= 0) setCurrentIndex(currentIndex - 4);
  };
  const nextRecoSlide = () => {
    if (recoIndex + 4 < recommendations.length) setRecoIndex(recoIndex + 4);
  };
  const prevRecoSlide = () => {
    if (recoIndex - 4 >= 0) setRecoIndex(recoIndex - 4);
  };

  const trackVisit = (productId) => {
    axios.post(
      `${process.env.REACT_APP_API_URL}/api/products/${productId}/visit`,
      {
  ...
  return (
    <div className="App">
      <header className="header-main">
        <img
          src={logoImg}
          alt="Logo Artisans du Maroc"
          className="header-logo-img"
        />
        <h1 className="logo">ARTISANS DU MAROC</h1>
        <div className="menu-wrap">
          <span className="status-badge">
            {user ? user.full_name.toUpperCase() : "VISITEUR"}
          </span>
          <button
            className="menu-btn"
          ...
          {isMenuOpen && (
            <div className="dropdown-menu">
              ...
              <Link to="/artisan-space" className="menu-item">
                🛠️ Espace Artisan
              </Link>
              <Link to="/admin/login" className="menu-item admin-item">
                ⚙️ Admin
              </Link>
            </div>
          )}
        ...
        <div className="search-overlay" onClick={closeResults}>
          <div className="search-popup" onClick={(e) => e.stopPropagation()}>
            <div className="search-popup-header">
              ...
              <button className="search-close" onClick={closeResults}>
                ✕
              </button>
            </div>
            {searching && (
              <p className="search-status">Recherche en cours...</p>
            )}
            {!searching && noResults && (
                  ...
                  {searchResults.products.map((p) => (
                    <div className="search-product-card" key={p.id}>
                      ...
                      <div className="search-product-info">
                        ...
                        <span className="search-artisan-tag">
                          🧑‍🎨 {p.artisan_name}
                        </span>
                      </div>
                      <button
        ...
        <section className="home-section">
          ...
          <div className="slider-container">
            ...
            <div className="artisans-grid">
              {recommendations.slice(recoIndex, recoIndex + 4).map((p) => (
                <div
                  ...
                  onClick={() => {
                    trackVisit(p.id);
                    navigate(`/artisan/${p.artisan_id}/products`);
                  }}
                >
                  ...
                  <div className="artisan-info">
                    ...
                    <p
                      className="city-tag"
                      style={{ color: "#b85c28", fontWeight: "700" }}
                    >
                      {convert(p.price)}
                    </p>
                    <p className="city-tag">🧑‍🎨 {p.artisan_name}</p>
                    <button
                      className="reco-info-btn"
                    ...
                    {p.phone_number && (
                      <a
                        href={`https://wa.me/${p.phone_number.replace(/\D/g, "")}?text=${encodeURIComponent(`Bonjour, je suis intéressé par votre produit: ${p.title}`)}`}
                        target="_blank"
            ...
            <button
              ...
              onClick={nextRecoSlide}
              disabled={recoIndex + 4 >= recommendations.length}
            >
              ›
            </button>
          </div>
        ...
        <div className="slider-container">
          ...
          <div className="artisans-grid">
            {artisans.slice(currentIndex, currentIndex + 4).map((art) => (
              ...
              >
                <div className="rating-tag">
                  ★ {Number(art.average_rating || 0).toFixed(1)}
                </div>
                <img
                  className="artisan-img"
                  src={imgSrc(art.profile_picture)}
                  alt={art.full_name}
                  onError={(e) =>
                    (e.target.src = "https://via.placeholder.com/400")
                  }
          ...
          <button
            className="slider-arrow right"
            onClick={nextSlide}
            disabled={currentIndex + 4 >= artisans.length}
          >
            ›
          </button>
        </div>
      ...
      {recoPopup && (
        <div className="reco-overlay" onClick={closeRecoPopup}>
          <div className="reco-popup" onClick={(e) => e.stopPropagation()}>
            <button className="reco-popup-close" onClick={closeRecoPopup}>
              ✕
            </button>

            <img
              src={imgSrc(recoPopup.image_url)}
              alt={recoPopup.title}
              className="reco-popup-img"
              style={{ cursor: "zoom-in" }}
              onClick={() =>
                setRecoLightbox({
                  url: imgSrc(recoPopup.image_url),
                  type: "image",
                })
              }
              onError={(e) =>
            ...
            <div className="reco-popup-body">
              ...
              <div className="reco-popup-specs">
                <span className="reco-spec-tag">
                  🎨 {recoPopup.couleur || "Non spécifié"}
                </span>
                <span className="reco-spec-tag">
                  ↕ H: {recoPopup.hauteur || "Non spécifié"}
                </span>
                <span className="reco-spec-tag">
                  ↔ L: {recoPopup.largeur || "Non spécifié"}
                </span>
              </div>
                ...
                <div className="reco-popup-gallery">
                  ...
                  <div className="reco-popup-gallery-grid">
                    {recoPopup.gallery.map((g) => (
                      <div
                        key={g.id}
                        className="reco-popup-gallery-item"
                        onClick={() =>
                          setRecoLightbox({
                      ...
                      >
                        {g.file_type === "video" ? (
                          <video
                            ...
                            className="reco-popup-gallery-thumb"
                            muted
                          />
                        ) : (
                          <img
              ...
              <p className="reco-popup-artisan-name">
                🧑‍🎨 {recoPopup.artisan_name}
              </p>
              ...
              <button
                className="reco-popup-contact"
                onClick={() => {
                  closeRecoPopup();
                  navigate(`/artisan/${recoPopup.artisan_id}/products`);
                }}
              >
      ...
      {recoLightbox && (
        <div className="reco-lightbox" onClick={() => setRecoLightbox(null)}>
          <button
            className="reco-lightbox-close"
            onClick={() => setRecoLightbox(null)}
          >
            ✕
          </button>
          {recoLightbox.type === "video" ? (
            <video
              src={recoLightbox.url}
              controls
              autoPlay
