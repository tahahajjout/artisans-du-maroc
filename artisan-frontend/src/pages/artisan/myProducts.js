...
function MyProducts() {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${process.env.REACT_APP_API_URL}/api/products/artisan/${userId}`)
      .then((res) => {
  ...
  const handleShowComments = async (product) => {
    ...
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_API_URL}/api/products/${product.id}/comments`,
      );
  ...
  const sidebarImg = imgSrc(user?.profile_picture);

  if (loading)
    return <div className="artisan-main">Chargement de vos créations...</div>;
            ...
            sortedProducts.map((p) => (
              <div className="product-card" key={p.id}>
                ...
                <div className="product-body">
                  ...
                  <div className="product-rating">
                    ...
                    <span className="rating-score">
                      {Number(p.average_rating).toFixed(1)} ({p.rating_count}{" "}
                      avis)
                    </span>
                  </div>
                ...
                <div className="product-actions">
                  <button
                    className="btn-card btn-comments"
                    onClick={() => handleShowComments(p)}
                  >
                    💬 Commentaires
                  </button>
                  <button
                    className="btn-card btn-modifier"
                    onClick={() => navigate(`/artisan/edit-product/${p.id}`)}
                  >
                    Modifier
                  </button>
                  <button
                    className="btn-card btn-supprimer"
      ...
      {deleteConfirm && (
        <div className="modal-overlay" onClick={() => setDeleteConfirm(null)}>
          ...
          >
            ...
            <p
              style={{ color: "#666", marginBottom: "28px", lineHeight: "1.6" }}
            >
                  ...
                  {commentsPopup.comments.map((c, i) => (
                    <tr key={i}>
                      ...
                      <td
                        ...
                        style={{ color: "#b35935", whiteSpace: "nowrap" }}
                      >
                        {renderStars(c.stars)} {c.stars}/5
                      </td>
                      <td className="comment-text">
                        {c.comment || <span style={{ color: "#ccc" }}>—</span>}
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            {galleryLoading ? (
              ...
              <>
                <div style={{ marginBottom: "16px" }}>
                  ...
                  <img
                    src={imgSrc(galleryPopup.product.image_url)}
                    alt="main"
                    onClick={() =>
                      setGalleryLightbox({
                        url: imgSrc(galleryPopup.product.image_url),
                        type: "image",
                      })
                    }
                    style={{
                ...
                {galleryPopup.gallery.length === 0 ? (
                  ...
                  <>
                    ...
                    >
                      Galerie ({galleryPopup.gallery.length} média
                      {galleryPopup.gallery.length > 1 ? "s" : ""})
                    </p>
